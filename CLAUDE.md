...
go run ./cmd/collector -dry-run            # fetch real feeds, print NDJSON, write nothing
...
- Pipeline: `Collector.Collect` runs every `Source` concurrently, each with its own timeout and with panics recovered. A `Source` returns the raw payload (`Raw{Data, Ext}`) and its articles, then `Normalize` (canonical URL, ID = first 16 hex chars of sha256(URL), plain-text title/summary, UTC), then `Tagger.Tag`, then `Store`.
